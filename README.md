...
- **주제별 핵심단어** (하단 메뉴 · 메인 화면): **최신 영상부터** 만들어 둔 이야기(최대 30편)를 훑어 주제(가족·관계, 돈·재산,
  일·직장, 감정·마음, 법·갈등, 건강·병원 …)를 나누고, **주제마다 핵심단어 20개를 자동으로 뽑아** 그 단어가 나오는 문장을 모아 줍니다.
  최근에 올라온 영상에 나온 단어일수록 위로 오고, 예문·문장도 최신 이야기 것부터 보여 줍니다.
  단어 · 문장(듣기·연속 듣기·익힘 표시) · 퀴즈는 이야기 학습과 똑같이 쓸 수 있고, 문장에는 어느 이야기에서 왔는지 함께 보여 줍니다.
  뜻이 없는 단어와 **번역이 비어 있던 예문 해석은 열 때 받아 채우고, 원래 이야기에도 저장**해 그 이야기에서도 채워져 보입니다.
- **단어장**: 저장한 단어 플래시카드 복습
...
public/            앱 화면 (Capacitor 웹 자산)
  ...
  lib/topics.js    주제별 핵심단어·관련 문장 모으기 (오프라인 계산)
